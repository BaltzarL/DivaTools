                ...
                try {
                    if (split[0].startsWith("TRITA")) {
                        ...
                        let lastName = fullName
                            ...
                            .join(" ")
                            .trim();
                        document.querySelector("input[id='addForm:authorSerie:0:autGiven']").value = firstName;
                        document.querySelector("input[id='addForm:authorSerie:0:autFamily']").value = lastName;


                        // Set bith year.
                        let personalNumber = split[2];
                        // Personal number can be YYMMDD-XXXX or YYYYMMDD-XXXX 
                        // This assumes all YYMMDD-XXXX is 19XX
                        let birthYear = personalNumber.length == 11 ? "19" + personalNumber.substr(0, 2) :
                            personalNumber.substr(0, 4);
                        document.querySelector("input[id='addForm:authorSerie:0:autBirthYear']").value = birthYear;

                        // TODO!
                        let program = split[3];
                        let courseCode = split[4];

                        // This does not work with multiple examinors/supervisors!
                        let examitorName = split[5];
                        let examitorFirstName = examitorName.split(" ")[0];
                        let examitorLastName = examitorName.split(" ").splice(1).join(" ").trim();
                        document.querySelector("input[id='addForm:examinerSerie:0:examinerGiven']").value = examitorFirstName;
                        document.querySelector("input[id='addForm:examinerSerie:0:examinerFamily']").value = examitorLastName;

                        let supervisorName = split[6];
                        let supervisorFirstName = supervisorName.split(" ")[0];
                        let supervisorLastName = supervisorName.split(" ").splice(1).join(" ").trim();
                        document.querySelector("input[id='addForm:supervisorSerie:0:supGiven']").value = supervisorFirstName;
                        document.querySelector("input[id='addForm:supervisorSerie:0:supFamily']").value = supervisorLastName;
                    }
                } catch (err) {
